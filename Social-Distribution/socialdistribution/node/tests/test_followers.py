...
class AuthorFollowerTest(TestCase):
    ...
    def test_list_follower(self):
        ...
        if login.status_code == 200:
            ...
            response = self.client.get(reverse('list_all_followers', kwargs={'author_id': 2}))
    ...
    def test_list_followers(self):
        ...
        if login.status_code == 200:
            ...
            # Test listing all followers
            response = self.client.get(reverse('list_followerf', kwargs={'author_ida': 2, 'follower_id': r'http%3A%2F%2Fdarkgoldenrod%2Fapi%2Fauthors%2F1'}))
            self.assertEqual(response.status_code, 200)
            self.assertContains(response, 'Author 1')
            self.assertNotContains(response, 'Author 3')
