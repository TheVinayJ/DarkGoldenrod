...
class ProfileTests(TestCase):
    def setUp(self):
        self.author1 = User.objects.create_user(id="1ce5e487-085c-4e8e-8c46-7b9045837828", display_name="Test Author1", description="Test Description", github="torvalds", email="author1@test.com", password="pass")
        self.author2 = User.objects.create_user(id="1ce5e487-085c-4e8e-8c46-7b9045837829", display_name="Test Author2", email="author2@test.com", password="pass")
    ...
    def test_author_content(self):
        ...
        if login.status_code == 200:
            response = self.client.get(reverse('profile', args=[self.author1.id]), follow=True)
    ...
    def test_friend_post_visibility_on_profile(self):
        ...
        if login.status_code == 200:
            response = self.client.get(
                reverse('profile', args=[self.author1.id]), follow=True)  # viewing their author1(who does not follow back) profile
            posts = response.context['posts']
    ...
    def test_api_successful_edit_profile(self):
        ...
        if login.status_code == 200:
            ...
            response = self.client.put(url, edit_changes, content_type='application/json', follow=True)
            self.assertEqual(response.status_code, 200)
