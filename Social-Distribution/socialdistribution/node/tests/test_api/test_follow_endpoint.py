...
class FollowersApiTest(TestCase):
    ...
    def test_get_followers_list(self):
        ...
        Follow.objects.create(follower=self.author_2, following=self.author_1, approved=True)
        Follow.objects.create(follower=self.author_3, following=self.author_1, approved=True)
        ...
        for i in range(2,4,1):
            ...
            self.assertTrue(followers[i]['id'].endswith("/api/authors/{i}"))
            self.assertEqual(followers[i]['host'], "http://localhost:8000/api/")
            self.assertEqual(followers[i]['displayName'], f"Test author {i}")
            self.assertEqual(followers[i]['github'], f"http://github.com/{i}")
            self.assertEqual(followers[i]['profileImage'], f"http://imagehost.com/author{i}.png")
            self.assertEqual(followers[i]['page'], f"http://localhost:8000/authors/{i}")
