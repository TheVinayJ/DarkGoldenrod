...
class CommentsApiTest(TestCase):
    def setUp(self):
        ...
        # Create a sample post by author1
        ...
        # Create a sample comment on the post by author2
    ...
    def test_post_comment_to_inbox(self):
        ...
        inbox_url = f"http://localhost:8000/api/authors/{self.author1.id}/inbox/"
        comment_data = {
            "type": "comment",
            "author": {
                "type": "author",
                "id": f"http://nodeaaaa/api/authors/{self.author2.id}",
                "displayName": self.author2.display_name,
                "github": self.author2.github,
                "profileImage":self.author2.profile_image.url if self.author2.profile_image else None,
                "page": f"http://nodeaaaa/authors/{self.author2.id}"
            },
            "text": "This is a new comment in the inbox",
            "published": timezone.now().isoformat(),
            "post": f"http://nodeaaaa/api/authors/{self.author1.id}/posts/{self.post.id}"
        }
    ...
    def test_get_comments_on_post(self):
        ...
        comments_url = f"http://localhost:8000/api/authors/{self.author1.id}/posts/{self.post.id}/comments/"
        ...
        self.assertEqual(response.data["src"][0]["author"]["id"], f"http://nodeaaaa/api/authors/{self.author2.id}")

    def test_get_specific_comment(self):
        """Test retrieving a specific comment by its full ID."""
        encoded_comment_id = f"http://nodeaaaa/api/authors/{self.author2.id}/commented/{self.comment.id}".replace("/", "%2F")
        comment_url = f"/api/authors/{self.author2.id}/commented/{self.comment.id}"
        ...
        self.assertEqual(response.data["author"]["id"], f"http://nodeaaaa/api/authors/{self.author2.id}")

    def test_get_comments_by_author(self):
        """Test retrieving a list of comments made by a specific author."""
        comments_list_url = f"http://localhost:8000/api/authors/{self.author2.id}/commented/"
        ...
        self.assertEqual(response.data["src"][0]["comment"], self.comment.comment)
        self.assertEqual(response.data["src"][0]["post"], f"http://nodeaaaa/api/authors/{self.author1.id}/posts/{self.post.id}")